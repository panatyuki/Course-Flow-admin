...
function EditCourse () {
  ...
  const lessonsRows = lessons.map((lesson, idx) => {
    return (
      <Table.Tr key={lesson.id} className={classes.tableRows}>
        <Table.Td>{idx+1}</Table.Td>
        <Table.Td>{lesson.title}</Table.Td>
        <Table.Td>{lesson.sublessons.length}</Table.Td>
        <Table.Td>
          <Group>
            <button onClick={() => deleteLesson(idx)} className={classes.actionButton}>
              <img src={imagesCourse.deleteIcon} alt='trash icon'/>
            </button>
            <button onClick={() => launchLessonEditor(idx)} className={classes.actionButton}>
  ...
  function addNewLesson() {
    // Simplified new lesson object; you might need more fields
    const newLesson = {
      index: lessons[lessons.length -1].index + 1,
      title: '',
      sublessons: [{title: '', videoUrl: '', videoResId: ''}],
      isNew: true
    };
  
    // Append new lesson to the state
    lessonsHandlers.append(newLesson);
  
    // If using internal state to toggle edit mode, set state to show editor for this new lesson
    setActiveLessonIndex(lessons.length); // Assuming `lessons.length` will give index of the newly added lesson
    toggleShowEditPage(true); // Assuming this function toggles whether the edit page is shown
  }
  ...
  if (!showEditPage) {
    return ( 
      <div>
          ...
          <form>
            ...
            /> : (
              <>
                ...
                <Button 
                  color='red'
                  onClick={() => handleDelete('trailerVid')}
                >Delete video
                </Button>
              </>
        ...
        <Group justify='space-between' className={classes.lessonsOperation}>
          <Title fw={500}>Lesson</Title>
          <button className={navClasses.button} onClick={addNewLesson}>
              + Add Lessons
          </button>
        </Group>
        <section className={classes.lessonsSection}>

          {/* prep the lessons table */}
          <Table className={classes.table}>
            <Table.Thead className={classes.tableHead}>
              <Table.Tr>
                <Table.Th className={classes.tableHeadTh}>&nbsp;</Table.Th>
                <Table.Th>Name</Table.Th>
                <Table.Th>Sublessons</Table.Th>
                <Table.Th className={classes.tableHeadTh}>Action</Table.Th>
              </Table.Tr>
            </Table.Thead>
    ...
    return (
      <EditCourseLesson 
        key={lessons[activeLessonIndex]?.id || `temp-${activeLessonIndex}`}
        lessonData={lessons}
